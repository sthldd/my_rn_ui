...
function Slide(Props: Props, State: State) {
  ...
  useEffect(() => {
    ...
    scrollView.current.scrollTo({
      x: Props.currentPage * Props.width,
      y: 0,
      animated: true,
    });
  }, []);
    ...
    for (let i = 0; i < Props.list.length; i++) {
      nodeArr.push(
        <View style={styles.imgContainer} key={i}>
          <Image
            source={{uri: Props.list[i].iconStyleicon}}
  ...
  const renderNextButton = () => {
    return (
      <TouchableOpacity onPress={nextButton} disabled={Props.disableNextButton}>
        <View>
          <Text style={Props.buttonText ? Props.buttonText : styles.buttonText}>
            ›
          </Text>
        </View>
      </TouchableOpacity>
  ...
  const renderPrevButton = () => {
    return (
      <TouchableOpacity onPress={prevButton} disabled={Props.disablePrevButton}>
        <View>
          <Text style={Props.buttonText ? Props.buttonText : styles.buttonText}>
            ‹
          </Text>
